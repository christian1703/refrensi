...
$sqlPerangkat = mysqli_query($con,"SELECT * FROM tb_perangkat
                      ...
                      INNER JOIN tb_jenisperangkat ON tb_perangkat.id_jenisperangkat=tb_jenisperangkat.id_jenisperangkat WHERE tb_perangkat.id_perangkat='$_GET[ID]'");
$d = mysqli_fetch_array( $sqlPerangkat);
 $time = time();
      header("Content-Type: application/vnd.msword");
      header("Expires: 0");
      header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
      header("content-disposition: attachment;filename=PERANGKAT_$time.doc");
?>
...
<body>
	<center>
		<h3>PERANGKAT PEMBELAJARAN <br><?=$d['jenis_perangkat']; ?></h3>
	</center>
